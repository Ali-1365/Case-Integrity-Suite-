...
export class PraxisService {
  ...
  async getRelevantPraxis(lawRefs: string[]): Promise<PraxisEntry[]> {
    try {
      ...
      if (!response.ok) throw new Error('Kunde inte hämta praxisdata i batch');

      const data = await response.json();
      const mappedResults: PraxisEntry[] = data.map((p: any) => ({
        id: p.id,
        reference: p.reference,
        linkedLaw: p.metadata.revisionNote || "",
        summary: p.text,
      ...
      // Ta bort dubbletter (även om batch-endpointen kan returnera dubbletter om flera söktermer träffar samma paragraf)
      return Array.from(new Map(mappedResults.map(item => [item.id, item])).values());
    } catch (error) {
