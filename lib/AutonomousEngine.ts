...
export class AutonomousEngine {
    ...
    private async processCycle() {
        ...
        try {
            ...
            // ⚡ Bolt Optimization: Batch execution for orchestrator.runFullAnalysis to avoid rate limits
            // and N+1 sequential bottlenecks. We process in chunks of 3.
            const chunkSize = 3;
            for (let i = 0; i < pendingDocs.length; i += chunkSize) {
                const chunk = pendingDocs.slice(i, i + chunkSize);
