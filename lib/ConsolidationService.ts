...
export class ConsolidationService {
  async consolidate(query: string, chain: ProvenanceChain): Promise<ConsolidationResult> {
    ...
    const lawRefs = chain.sources.map(s => `${(s as { sourceCode: string }).sourceCode} ${(s as { chapter: string | number }).chapter ? (s as { chapter: string | number }).chapter + ':' : ''}${(s as { section: string | number }).section}`);
    ...
    const result: ConsolidationResult = {
      ...
      hierarchy: {
        ...
        constitution: chain.sources.filter(s => ['BK', 'RF', 'TF', 'YGL'].includes((s as { sourceCode: string }).sourceCode)).sort((a, b) => {
          if ((a as { sourceCode: string }).sourceCode === 'BK') return -1;
          if ((b as { sourceCode: string }).sourceCode === 'BK') return 1;
          return 0;
        }),
        law: chain.sources.filter(s => !['BK', 'RF', 'TF', 'YGL'].includes((s as { sourceCode: string }).sourceCode)),
        regulation: [],
        praxis: relevantPraxis
      },
      interplayAnalysis: "Hierarkisk analys initierad enligt Oracle v.7.6-GOLD (Lex Superior).",
      affectedNorms: Array.from(new Set(chain.sources.map(s => (s as { sourceCode: string }).sourceCode))),
      provenanceHashes: [
        ...chain.sources.map(s => (s as { provenanceHash: string }).provenanceHash),
        ...relevantPraxis.map(p => (p as { provenanceHash: string }).provenanceHash)
      ],
      riskReport
    };
    ...
    await auditService.log({
      ...
      actor: 'SYSTEM',
      affectedLaws: chain.sources.map(s => `${(s as { sourceCode: string }).sourceCode} ${(s as { sfsNumber: string }).sfsNumber}`),
      provenanceHashes: (result as { provenanceHash: string }).provenanceHashes,
